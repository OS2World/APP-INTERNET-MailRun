     ...
     pMainFormInfo=(PMAINFORMINFO) WinQueryWindowULong(hwndDlg, QWL_USER);
      ...
      if( strlen(mailServer) > 0 && strlen(userName) > 0 && strlen(passWord) > 0 )
         ...
         if( pgmData[5] ) 
            ...
            //                 PP_FONTNAMESIZE,
            //                 strlen (fnt) + 1, fnt) ;
            // WinSetWindowText(WinWindowFromID(hwndDlg, 1000), "ON");
            chkdat1.hwndDlg = hwndDlg;
            mailthread = _beginthread(checkForMail, NULL, 16384, (PVOID)&chkdat1);
            // WinEnableControl(hwndDlg, 1000, FALSE);
            }
         else
            {
            bkgndcolor = CLR_RED;   
...
HWND OpenMain(HWND hwndOwner, PVOID pInfo)
{
   ...
   hwndFrame=WinCreateWindow(HWND_DESKTOP, WC_FRAME, "Mail Run ver. 1.00", 0,0,0,0,0,hwndOwner,HWND_TOP,ID_MAINFORM,&frameData,0);
   if (hwndFrame) {
